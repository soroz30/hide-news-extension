    ...
    <div className="App">
      ...
      <body>
        <div>
          <ul id="wordsList">
            {words.map((word) => {
              return <li>{word} <span onClick={() => removeWord(word)} style={{cursor: "pointer"}}>X</span></li>
            })}
          </ul>
        </div>
        <div>
          <input id="addWordInput" value={input} onChange={handleInputChange} />
          <button id="buttonAdd" onClick={addWord}>Add Word</button>
        </div>
        <div>
          <button id="buttonShow" onClick={showUnwanted}>Show Links</button>
        </div>
        <div>
          <button id="buttonHide" onClick={hideUnwanted}>Hide Links</button>
        </div>
      </body>
    </div>
  )
